	...
	Begin {
		...
		namespace QueryUser
		{
			...
			public class TerminalSessionInfo
			{
				...
				public bool IsUserSession; public bool IsLocalAdmin; public bool IsDomainAdmin; public DateTime? LogonTime; public TimeSpan? IdleTime; public DateTime? DisconnectTime;
...
'@
        ...
        Function IsDomainAdmin{
            ...
            try
            {
                try
                {
                    $domainadmins = (Get-ADUser -Filter "memberOf -RecursiveMatch '$((Get-ADGroup "Domain Admins").DistinguishedName)'").samaccountname
                }
                catch
                {
                    write-warning "Error retrieving domain admin list"
                    
                }

                if($username -match '\*')
                {
                    $username = Get-ADUser -Filter "name -like '$username'" | select -ExpandProperty samaccountname
                }
	...
	Process {
        ...
        foreach($computername in $name)
        {
		    ...
		    Try
            {
			    ...
			    [psobject[]]$TerminalSessions = [QueryUser.Session]::ListSessions($computername)
			    ForEach ($TerminalSession in $TerminalSessions)
                {
				    If (($TerminalSession.IsUserSession))
                    {
					    [psobject]$SessionInfo = [QueryUser.Session]::GetSessionInfo($computername, $TerminalSession.SessionId)
					    If ($SessionInfo.UserName)
                        {
                            $SessionInfo.IsDomainAdmin = IsDomainAdmin($SessionInfo.UserName)
